...
test("creates a new question when the form is submitted", async () => {
  ...
  // Wait for first render of list to avoid state warnings
  await screen.findByText(/lorem testum 1/g);
  ...
  // (Answer 3 and Answer 4 remain blank if not required.)
  fireEvent.change(screen.queryByLabelText(/Correct Answer/), {
    target: { value: "1" },
  });
  ...
  // Switch back to question list
  fireEvent.click(screen.queryByText(/View Questions/));

  // Use a flexible matcher to check for "Test Prompt" in the rendered element
  expect(
    await screen.findByText((content) => content.includes("Test Prompt"))
  ).toBeInTheDocument();
  expect(await screen.findByText(/lorem testum 1/g)).toBeInTheDocument();
});
